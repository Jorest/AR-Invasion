...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.ARStarterAssets
{
    ...
    public class ARInteractorSpawnTrigger : MonoBehaviour
    {
        ...
        //ADDED BY ME SO IT CAN BE BLOCK HORIZONRTALLY 
        [SerializeField]  bool BlockHorizontalSpawn;
        ...
        void Start()
        {
            gameStarter = GameManager.Instance;
        ...
        void Update()
        {
            ...
            if (attemptSpawn && m_ARInteractor.TryGetCurrentARRaycastHit(out var arRaycastHit))
            {
                ...
                if (!gameStarter.EnablePortalSpawn)
                    return;
                var arPlane = arRaycastHit.trackable as ARPlane;
                if (arPlane == null)
                    return;
                // checking for the orizontal mod if needed
                if (BlockHorizontalSpawn && arPlane.alignment == PlaneAlignment.HorizontalUp)
                    return; 
                if (m_RequireHorizontalUpSurface && arPlane.alignment != PlaneAlignment.HorizontalUp)
                    return;
                if (_blockSpawn)
                    return;
                bool spawned = m_ObjectSpawner.TrySpawnObject(arRaycastHit.pose.position, arPlane.normal);
                
                if (spawned)
                {
                    gameStarter.SetPortalEnable(false);         
                }
